...
interface ClientKeyPatch {
  /**
   * The `key` field only exists on `@mediaurl/client` responses.
...
interface ClientSourcePatch {
  /**
   * The `kind` field only exists on `@mediaurl/client` responses.
...
export type AddonCallOptions = {
  /**
   * MediaURL signature. Don't forget to refresh this signature regularly.
   */
  signature?: string;
