...
import { EndpointType, analyzeEndpoints } from "./utils/analyzeEndpoints";
  ...
  public async load({
    ...
    const loadViaRepository = async (
      ...
      try {
        ...
        for (const props of res) {
          ...
          handleRequirement(
            { legacyId: props.id, endpoints: props.endpoints },
            "addon",
            false,
            requirePath,
          );
    ...
    const loadViaEndpoints = async (
      ...
      try {
        ...
        if (!result) {
          throw new Error("No MediaURL addon found");
        }
        for (const r of result) {
          if (r.props) {
            ...
            handleRequirement(
              { legacyId: null, endpoints: r.endpoints },
    ...
    for (const input of inputs) {
      if (input.addonClass) {
        ...
        handleRequirement(
          { legacyId: null, endpoints: input.endpoints },
          "unknown",
          true,
          [],
          rootIndex++,
        ...
        handleRequirement(
          { legacyId: null, endpoints: [input.url] },
          "unknown",
          true,
          [],
          rootIndex++,
        ...
        if (!ignore.has(input.userInput)) {
          handleRequirement(
            { legacyId: null, endpoints: mutateUserInput(input.userInput) },
            "unknown",
            true,
            [],
            rootIndex++,
